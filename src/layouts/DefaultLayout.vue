...
import AppToast from '../components/AppToast.vue' // Certifica-te que o caminho está correto
...
// --- LÓGICA DE SETTINGS (Migrada da View) ---
const isSettingsOpen = ref(false)
const localCurrency = ref(store.state.currency)
const showToast = ref(false)

function toggleSettings() {
  // Sincroniza com a store sempre que abre
...
function saveChanges() {
  ...
  isSettingsOpen.value = false // Fecha o menu ao salvar
...
function resetToDefaults() {
  ...
  // Não salva automaticamente, o utilizador ainda tem de clicar em Save se quiser confirmar, 
  // ou podes chamar saveChanges() aqui diretamente se preferires comportamento imediato.
  // Vou assumir comportamento imediato para ser mais fluido no dropdown:
  saveChanges() 
}

// --- LÓGICA DO GLIDER (Mantida) ---
const updateGlider = () => {
...
watch(() => route.path, () => { setTimeout(updateGlider, 0) }, { flush: 'post', immediate: true })
</script>
      ...
      <nav>
        ...
        <div class="nav-center" ref="navCenter">
          ...
          <RouterLink to="/anomalies">
            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M12 3v2"/><path d="M12 5c-3.31 0 -6 2.69 -6 6l0 6c-1 0 -2 1 -2 2h8M12 5c3.31 0 6 2.69 6 6l0 6c1 0 2 1 2 2h-8"/><path d="M10 20c0 1.1 0.9 2 2 2c1.1 0 2 -0.9 2 -2"/></g></svg>
            <span>Anomalies</span>
          </RouterLink>
        ...
        <div class="nav-right">
          ...
          <div v-if="isSettingsOpen" class="settings-dropdown">
            ...
            <div class="currency-row">
               <button :class="{ active: localCurrency === 'USD' }" @click="localCurrency = 'USD'">USD($)</button>
               <button :class="{ active: localCurrency === 'EUR' }" @click="localCurrency = 'EUR'">EUR(€)</button>
               <button :class="{ active: localCurrency === 'GBP' }" @click="localCurrency = 'GBP'">GBP(£)</button>
            </div>
...
.btn-reset:hover { background-color: #fff0f0; }
